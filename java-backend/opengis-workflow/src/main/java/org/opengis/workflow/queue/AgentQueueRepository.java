...
public final class AgentQueueRepository {
  ...
  private AgentQueueItem fromJson(JsonNode node) {
    ...
    return new AgentQueueItem(
        ...
        workspace,
        node.path("conversation_id").asText(),
        node.path("profile_name").asText("gis-build"),
        metadataNode.path("connection_id").asText(),
        metadataNode.path("workflow_id").asText(),
        QueueStatus.fromWire(node.path("status").asText("queued")),
