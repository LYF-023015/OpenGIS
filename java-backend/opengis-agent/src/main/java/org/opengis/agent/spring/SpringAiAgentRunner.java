...
import org.opengis.agent.loop.AgentStopException;
import org.opengis.agent.loop.ContinuationPolicy;
...
import org.opengis.ai.spring.DefinitionOnlyToolCallback;
import org.opengis.ai.spring.SpringAiMessages;
...
public final class SpringAiAgentRunner {
  ...
  private void publishFinalAnswer(AgentRunContext context, String answer, String source) {
    if (contexts.messages(context.conversationId()).stream()
        .noneMatch(
            message ->
                message.role() == org.opengis.ai.model.LlmRole.ASSISTANT
