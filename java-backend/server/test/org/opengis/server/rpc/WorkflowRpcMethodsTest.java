...
class WorkflowRpcMethodsTest {
  ...
  void registersQueueAndWorkflowMethodsAndPersistsV2(@TempDir Path workspace) {
    ...
    assertThat(saved.path("status").asString()).isEqualTo("ok");
    JsonNode loaded =
        mapper.valueToTree(
            registry
  ...
  void inspectReturnsManualRequiredForPythonV1() throws Exception {
    ...
    assertThat(result.path("status").asString()).isEqualTo("manual_required");
  }
}
