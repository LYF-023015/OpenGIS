...
class WorkflowMessagePartProjectionTest {
  ...
  private static void awaitTerminal(
    ...
    String status = service.status(workspace, runId);
    while (!terminal.contains(status) && System.nanoTime() < deadline) {
      Thread.sleep(20);
      status = service.status(workspace, runId);
    }
    assertThat(status).as("workflow %s should reach a terminal state", runId).isIn(terminal);
  }
