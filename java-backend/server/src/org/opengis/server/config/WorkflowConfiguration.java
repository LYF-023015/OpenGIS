...
public class WorkflowConfiguration {
  ...
  WorkflowApplicationService workflowApplicationService(
    ...
    return new WorkflowApplicationService(
        agentExecutor, agents, tools, ui, mapper, executionServices, executionBridge);
  }
  ...
  AgentQueueService agentQueueService(
    ...
    return new AgentQueueService(
        new QueueRunPort() {
          @Override
          public StartResult start(org.opengis.automation.workflow.queue.AgentQueueItem item, boolean resume) {
            if (item.workflowId() != null && !item.workflowId().isBlank()) {
              var workflow =
                  new org.opengis.automation.workflow.persistence.WorkflowStore(item.workspace())
          ...
          @Override
          public boolean cancel(org.opengis.automation.workflow.queue.AgentQueueItem item) {
          ...
          @Override
          public String status(org.opengis.automation.workflow.queue.AgentQueueItem item) {
            ...
            return RunArchive.load(item.workspace(), item.runId())
                .map(archive -> archive.meta().path("status").asString("unknown"))
                .orElse("unknown");
          }
        });
  }
