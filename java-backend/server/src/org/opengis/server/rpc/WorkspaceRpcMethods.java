...
import org.opengis.assistant.agent.persistence.RunIndex;
...
/** Workspace state, run archive, permission and migration RPC methods. */
@Component
public class WorkspaceRpcMethods {
  private final RpcMethodRegistry registry;
  private final ObjectMapper objectMapper;

  public WorkspaceRpcMethods(RpcMethodRegistry registry, ObjectMapper objectMapper) {
  ...
  private Object inbox(JsonNode params) {
    String status = params.path("status").asString();
    List<JsonNode> values =
        limitedValues(new SessionStore(workspace(params)).load().path("inbox"), 10_000);
    if (!status.isBlank()) {
      values =
          values.stream().filter(item -> status.equals(item.path("status").asString())).toList();
    }
  ...
  private Object addPermissionRule(JsonNode params) {
    ...
    if (params.path("profile_name").isString()) {
      rule.put("profile_name", params.path("profile_name").asString());
    } else {
  ...
  private Object runs(JsonNode params) {
    return Map.of(
        ...
        RunArchive.list(workspace(params)).stream()
            ...
            .map(WorkspaceRpcMethods::runIndex)
            .toList());
  }
  ...
  private Object revertRun(JsonNode params) {
    ...
    RunArchive run =
        RunArchive.load(workspace, runId)
            .orElseThrow(() -> invalidParams("run archive not found: " + runId));
    String preSha = run.meta().path("pre_sha").asString();
    if (preSha.isBlank()) {
