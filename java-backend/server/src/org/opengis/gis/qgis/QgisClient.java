...
public final class QgisClient {
  ...
  public JsonNode call(String command, JsonNode params, CancellationSignal cancellation) {
    if (!COMMANDS.contains(command)) {
      throw new GisException("qgis_unknown_command", "Unknown QGIS command: " + command);
    }
    ...
    try (Socket socket = new Socket()) {
      ...
      if (size <= 0 || size > MAX_RESPONSE_BYTES) {
        throw new GisException("qgis_invalid_frame", "Invalid QGIS response length: " + size);
      }
      byte[] response = input.readNBytes(size);
      if (response.length != size)
        throw new GisException("qgis_connection_closed", "QGIS closed the connection");
      cancellation.throwIfCancelled();
      ...
      throw new GisException("qgis_connection_failed", "Cannot connect to QGIS MCP", exception);
    }
  }
  ...
  private static void requireLoopback(String host) {
    try {
      if (!InetAddress.getByName(host).isLoopbackAddress()) {
        throw new GisException("unsafe_qgis_host", "QGIS MCP host must resolve to loopback");
      }
    } catch (GisException exception) {
      throw exception;
    } catch (Exception exception) {
      throw new GisException("invalid_qgis_host", "Cannot resolve QGIS MCP host", exception);
    }
  }
}
