...
public final class OsmAdapter {
  ...
  public JsonNode call(
    ...
    return switch (command) {
      ...
      default -> throw new ToolException("osm_unknown_command", "Unknown OSM command: " + command);
    };
  }

  private JsonNode downloadFeatures(
      JsonNode params, Path workspace, CancellationToken cancellation) {
    JsonNode results = searchParams(text(params, "place"), 1, timeout(params), cancellation);
    if (results.isEmpty())
      throw new ToolException("osm_place_not_found", "OSM place was not found");
  ...
  private JsonNode search(JsonNode params, CancellationToken cancellation) {
  ...
  private JsonNode searchParams(
      String query, int limit, Duration timeout, CancellationToken cancellation) {
    ...
    try {
      ...
      if (!result.isArray())
        throw new ToolException("osm_invalid_response", "Invalid Nominatim response");
      return result;
    } catch (ToolException exception) {
      throw exception;
    } catch (Exception exception) {
      throw new ToolException("osm_invalid_response", "Cannot parse Nominatim response", exception);
    }
  }

  private ObjectNode query(String raw, Duration timeout, CancellationToken cancellation) {
    ...
    try {
      ...
      throw new ToolException("osm_invalid_response", "Cannot parse Overpass response", exception);
    }
  }

  ObjectNode osmToGeoJson(JsonNode response, CancellationToken cancellation) {
  ...
  private static String buildBboxQuery(JsonNode params) {
    ...
    if (south >= north || west >= east)
      throw new ToolException("osm_invalid_bbox", "Invalid OSM bbox ordering");
    String key = text(params, "key");
    if (!TAG.matcher(key).matches())
      throw new ToolException("osm_invalid_tag", "Invalid OSM tag key");
    String type = optionalText(params, "geometry_type", "way").toLowerCase(Locale.ROOT);
    if (!GEOMETRY_TYPES.contains(type))
      throw new ToolException("osm_invalid_geometry_type", "Invalid OSM geometry type");
  ...
  private static double coordinate(JsonNode params, String field, double min, double max) {
    if (!params.path(field).isNumber())
      throw new ToolException("osm_invalid_bbox", "Missing bbox field: " + field);
    double value = params.path(field).asDouble();
    if (value < min || value > max)
      throw new ToolException("osm_invalid_bbox", "Invalid bbox field: " + field);
    return value;
  }
  ...
  private static String text(JsonNode params, String field) {
    ...
    if (value.isBlank())
      throw new ToolException("osm_invalid_params", "Missing OSM parameter: " + field);
    return value;
  }
