...
class Phase8ServicesTest {
  @Test
  void toolCatalogExposesAllOperationCodeAndWorkerLifecycleNames() {
    ObjectMapper mapper = new ObjectMapper();
    Phase8ExecutionBridge bridge = new Phase8ExecutionBridge(mapper);
    Phase8Services services = new Phase8Services(mapper, bridge);
    try {
      assertThat(
              Phase8ToolCatalog.create(mapper, services, bridge).stream()
  ...
  @Test
  void directJavaScriptPathCompilesRunsArchivesAndReturnsLegacyShape(@TempDir Path workspace) {
    ObjectMapper mapper = new ObjectMapper();
    Phase8ExecutionBridge bridge = new Phase8ExecutionBridge(mapper);
    Phase8Services services = new Phase8Services(mapper, bridge);
    try {
