...
/** Script, reusable operation and worker execution RPC methods. */
@Component
public final class ExecutionRpcMethods {
  ...
  public ExecutionRpcMethods(
      RpcMethodRegistry methods,
      ExecutionServices services,
      ScriptExecutionBridge bridge,
      UiRpcGateway ui,
      ObjectMapper mapper) {
  ...
  private Object cancelScript(JsonNode params) {
    return services.cancelScript(params.path("run_id").asString(""));
  }

  private Object listScripts(JsonNode params) {
    return services.listScripts(
        workspace(params), params.path("query").asString(""), params.path("limit").asInt(50));
  }
  ...
  private Object listOperations(JsonNode params) {
    return services
        .operations()
        .list(workspace(params), params.path("query").asString(""), params.path("limit").asInt(50));
  }
  ...
  private Object promoteOperation(JsonNode params) {
    return services
        ...
        .promoteScript(
            ...
            params.path("operation_id").asString(""),
            params.path("overwrite").asBoolean(false));
  }
    ...
    return Map.of(
        ...
        services
            ...
            .pause(
                workspace(params),
                required(params, "worker_id"),
                params.path("reason").asString("ui_pause")));
  }
  ...
  private Object migrateWorker(JsonNode params) {
    return services.migrateWorkers(workspace(params), params.path("worker_id").asString(""));
  }

  private ToolExecutionContext context(JsonNode params) {
    return new ToolExecutionContext(
        workspace(params),
        params.path("run_id").asString("direct"),
  ...
  private static Path workspace(JsonNode params) {
    String value = params.path("workspace_path").asString(params.path("workspace").asString(""));
  ...
  private static String required(JsonNode params, String name) {
    String value = params.path(name).asString();
