...
@Component
public final class Phase7RpcMethods {
  private final RpcMethodRegistry registry;
  private final RasterService rasters;
  private final ObjectMapper mapper;
  private final VectorLoader vectors;

  public Phase7RpcMethods(RpcMethodRegistry registry, RasterService rasters, ObjectMapper mapper) {
  ...
  private Object inspect(JsonNode params) {
    ...
    Path path = WorkspaceGisPaths.input(workspace, params.path("path").asText());
    GisFormat format =
  ...
  private Object registerRaster(JsonNode params) {
    ...
    Map<String, Object> style =
        params.path("style").isObject()
            ? mapper.convertValue(params.path("style"), Map.class)
            : Map.of();
