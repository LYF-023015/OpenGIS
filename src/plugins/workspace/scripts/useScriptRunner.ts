/** 文件职责：script-runner 前端功能：承载该领域的核心业务流程。 */
/**
...
export interface UseScriptRunner {
  ...
  run: (
    code: string,
    opts?: { workspacePath?: string | null; execTimeout?: number },
  ) => Promise<void>;
  stop: () => Promise<void>;
  clearOutput: () => void;
}
...
export function useScriptRunner(): UseScriptRunner {
  ...
  const pushChunk = useCallback(
    (stream: OutputChunk["stream"], text: string) => {
      if (!text) return;
      setChunks((prev) => [
        ...prev,
  ...
  const run = useCallback(
    async (
      ...
      try {
        const reply = await backendClient.send<
          ScriptResult & { run_id?: string }
        >("rpc.code.run_script", {
          run_id: id,
          code,
