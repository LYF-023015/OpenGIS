/** 文件职责：作为 MapView 页面主入口，编排界面状态和子组件。 */
...
import { useSettingsStore } from "@/plugins/system/settings/model/settingsStore"; // 新增：导入settingsStore
import { useT } from "@/app/i18n";
import { BUILTIN_BASEMAPS } from "@/shared/geo"; // 新增：导入底图列表
import { mapEngine } from "./engine/MapEngine";
import { loadGeoFiles } from "@/shared/geo";
import { useIdentify } from "./identify/useIdentify";
import { useBoxSelect } from "./identify/useBoxSelect";
import { FeatureAttributePanel } from "./identify/FeatureAttributePanel";
import { BoxSelectResultPanel } from "./identify/BoxSelectResultPanel";
import { ExportButton } from "./export/ExportButton";
import { PinnedImagePanel } from "./components/PinnedImagePanel";
...
export function MapView({
  ...
  const clearBoxSelectedFeatures = useMapStore(
    (s) => s.clearBoxSelectedFeatures,
  );
  ...
  useEffect(() => {
    ...
    const renderTypes = prevRenderTypeRef.current;
    const layerData = prevLayerDataRef.current;
    ...
    // 还在".)
    map.on("load", () => {
      const currentLayers = useMapStore
        .getState()
        .layers.filter((l) => !l.extension);
      ...
      for (const layer of currentLayers) {
        try {
          ...
          console.error(
            "[MapView] initial seed sync failed, layer:",
            layer.id,
            layer.name,
            err,
          );
    ...
    mapEngine.setOnStyleReload(() => {
      ...
      for (const layer of currentLayers) {
        try {
          ...
          renderTypes.set(layer.id, layer.style.renderType);
          layerData.set(layer.id, layer.data);
          syncedIds.push(layer.id);
        } catch (err) {
          // 与 seed 相同：style reload 后单层失败不能中断整体重挂，
          // 否则 refs 与地图脱节，后续 diff 永远不再补挂。
          console.error(
            "[MapView] style reload sync failed, layer:",
            layer.id,
            layer.name,
            err,
          );
  ...
  useEffect(() => {
    ...
    if (
      !showMapLabels &&
      target.type === "vector-style" &&
      !target.id.endsWith("-nolabels")
    ) {
  ...
  }, [persistedBasemapId, showMapLabels, basemap.id]);
      ...
      for (const layer of currentLayers) {
        try {
          ...
          if (!prevIds.has(layer.id)) {
            ...
            prevRenderType &&
            prevRenderType !== layer.style.renderType
          ) {
          ...
          // 后续任何 store 变更都不会再补挂。
          console.error(
            "[MapView] sync layer failed, layer:",
            layer.id,
            layer.name,
            err,
          );
      ...
      {showControls && (
        <div className="absolute top-3 left-3 z-10 flex items-center gap-2">
          <div className="glass rounded-lg px-3 py-1.5 text-xs text-text-secondary">
            <span className="text-accent-geo font-display font-semibold">
              OpenGIS
            </span>
          ...
          <button
            ...
            title={
              boxSelectActive ? t.map.switchToPan : t.map.switchToBoxSelect
            }
          >
