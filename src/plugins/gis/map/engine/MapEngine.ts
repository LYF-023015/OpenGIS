 ...
 * `src/plugins/gis/map/renderers/` 下，每种渲染模式一个独立文件。MapEngine
 * 这里只做：
...
export class MapEngine {
  ...
  syncLayer(layer: MapLayerDefinition): void {
    if (!this.map) {
      console.warn(
        "[MapEngine] syncLayer called but map is null, layer:",
        layer.id,
        layer.name,
      );
      return;
    ...
    const managesOwnSource = ["cluster", "raster"].includes(
      layer.style.renderType,
    );
  ...
  removeMapLayer(layerId: string): void {
    if (!this.map) {
      console.warn(
        "[MapEngine] removeMapLayer called but map is null, layerId:",
        layerId,
      );
      return;
    }
  ...
  removeRenderLayersOnly(layerId: string): void {
    if (!this.map) {
      console.warn(
        "[MapEngine] removeRenderLayersOnly called but map is null, layerId:",
        layerId,
      );
      return;
    }
  ...
  flyTo(
    center: [number, number],
    zoom?: number,
    options?: Omit<CameraUpdate, "center" | "zoom">,
  ): void {
    if (!this.map) return;
    this.map.flyTo({
      center,
  ...
  private ensureGeoJSONSource(layer: MapLayerDefinition): void {
    ...
    const existingSource = this.map.getSource(sourceId) as
      | (maplibregl.GeoJSONSource & {
          ...
          updateData?: (
            diff: NonNullable<ParsedVectorData["runtimeDiff"]>,
          ) => maplibregl.GeoJSONSource;
        })
      | undefined;
    ...
    if (existingSource) {
      ...
      const preferFullSetData =
        Boolean(layer.meta.dynamic) &&
      ...
      if (
        ...
        try {
          ...
          console.warn(
            "[MapEngine] GeoJSONSource.updateData failed, falling back to setData:",
            err,
          );
        }
      }
