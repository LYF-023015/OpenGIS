...
import MarkdownBlock from "@/plugins/assistant/chat/messages/MarkdownBlock";
...
export function OperationEditorView({ tab }: { tab: ViewTab }) {
  ...
  const loadDetail = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await backendClient.send<{ operation?: OperationDetail }>(
        "rpc.operations.get",
        {
    ...
    <div className="operation-workbench scrollbar-none h-full min-h-0 overflow-y-auto">
      <div className="sticky top-0 z-10 flex items-center gap-3 bg-bg-primary/95 px-5 py-3 backdrop-blur app-region-drag">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 truncate text-xs text-text-muted">
            {detail.description || t.operations.noDescription}
          </p>
        </div>
      </div>
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-4 p-5">
          ...
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.35fr_1fr]">
            <div className="min-w-0">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs md:grid-cols-4">
                <InfoPill
                  label={t.operations.version}
                  value={detail.version || "-"}
                />
                <InfoPill
                  label="Revision"
                  value={String(detail.revision ?? "-")}
                />
                <InfoPill
                  label={t.operations.entry}
                  value={detail.entry || "Operation.java"}
                />
            ...
            <div className="min-w-0">
              ...
              <div className="grid grid-cols-1 gap-2 text-xs sm:grid-cols-2">
                <InfoPill
                  label="Language"
                  value={detail.runtime?.language || "java"}
                />
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {(dependencies.length
                  ? dependencies
                  : [t.operations.noDependencies]
                ).map((dependency) => (
                  <span
                    key={dependency}
                    className="max-w-full truncate rounded bg-[var(--operation-card-soft)] px-2 py-1 text-2xs text-text-muted"
                  >
                    {dependency}
                  </span>
                ))}
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
          <JsonBlock
            title={t.operations.inputSchema}
            value={detail.input_schema || {}}
          />
          <JsonBlock
            title={t.operations.outputSchema}
            value={detail.output_schema || {}}
          />
        </div>
...
function getOperationId(tab: ViewTab): string {
  ...
  if (parts[parts.length - 1] === "operation.json")
...
function StatusBadge({ status }: { status?: string }) {
  ...
  return (
    <span
      className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] leading-none ${tone}`}
    >
      {status || "draft"}
    </span>
  );
...
function InfoPill({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0 rounded-md bg-[var(--operation-card-soft)] px-2.5 py-2">
      ...
      <div className="truncate text-text-secondary" title={value}>
        {value}
      </div>
    </div>
  );
...
function SectionTitle({
  ...
  return (
    <div
      className={`mb-2 text-2xs font-semibold uppercase tracking-wider text-text-muted ${className}`}
    >
      {children}
    </div>
  );
...
function StateMessage({
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center bg-bg-primary px-5 text-center text-text-muted">
      <Icon
        className={`mb-2 h-6 w-6 ${icon === "loading" ? "animate-spin" : ""}`}
      />
      <div className="text-sm font-medium text-text-secondary">{title}</div>
      {body && (
        <div className="mt-1 max-w-md text-xs leading-relaxed">{body}</div>
      )}
    </div>
  );
}
