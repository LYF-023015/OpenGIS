/** 文件职责：approval 前端功能：实现该文件名所对应的单一职责。 */
...
} from "@/plugins/assistant/approval/model/approvalStore";
...
function ApprovalCard({
  ...
  const rememberAndAllow = async () => {
    ...
    try {
      ...
      import("@/plugins/automation/runs/model/runsStore").then(({ useRunsStore }) => {
        useRunsStore
          .getState()
          .refreshControlPlane()
          .catch(() => {});
  ...
  return (
    ...
    >
      <div className={headerClass}>
        <div
          className={`${iconWrapClass} ${request.danger ? "text-accent-danger" : "text-accent-primary"} ${embedded ? "" : request.danger ? "bg-accent-danger/10" : "bg-accent-primary/10"}`}
        >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            {request.toolName && (
              <span
                className={`${embedded ? "approval-muted px-1 py-0.5 text-[10px]" : "approval-muted border approval-border px-1.5 py-0.5 text-2xs"} shrink-0 rounded font-mono text-text-secondary`}
              >
                {request.toolName}
              </span>
            )}
        ...
        {queueLength > 0 && (
          <span
            className={`${embedded ? "px-1.5 py-0.5 text-[10px]" : "px-2 py-1 text-2xs"} approval-muted rounded-full text-text-muted`}
          >
            队列 +{queueLength}
          </span>
        )}
      ...
      <div className={bodyClass}>
        {riskItems.length > 0 && (
          <div
            className={`${embedded ? "mb-2 px-2 py-1.5" : "mb-3 border border-accent-warning/25 px-3 py-2"} rounded-md bg-accent-warning/10`}
          >
            <div
              className={`${embedded ? "mb-1 text-2xs" : "mb-1.5 text-xs"} flex items-center gap-1.5 font-medium text-accent-warning`}
            >
            ...
            <ul className="space-y-1">
              {riskItems.map((risk, index) => (
                <li
                  key={`${risk}-${index}`}
                  className={`${embedded ? "gap-1.5 text-2xs leading-snug" : "gap-2 text-xs leading-relaxed"} flex text-text-secondary`}
                >
                  <ChevronRight
                    className={`${actionIconClass} mt-0.5 shrink-0 text-accent-warning`}
                  />
      ...
      <div className={footerClass}>
        <button onClick={deny} className={secondaryButtonClass}>
        ...
        <button
          ...
          disabled={
            busy !== null || (request.kind === "choose" && !value.trim())
          }
...
function CodePreview({
  ...
  return (
    <div className="approval-muted border border-accent-primary/25 overflow-hidden rounded-md">
      <div
        className={`${embedded ? "gap-1.5 px-2.5 py-1.5" : "gap-2 border-b approval-border px-3 py-2"} flex items-center`}
      >
        <Code2
          className={`${embedded ? "h-3 w-3" : "h-3.5 w-3.5"} text-text-muted`}
        />
        <span
          className={`${embedded ? "text-2xs" : "text-xs"} font-medium text-text-secondary`}
        >
          Java code
        </span>
        {header && (
      ...
      <pre
        className={`${embedded ? "max-h-[150px] p-2.5 text-2xs leading-snug" : "max-h-[320px] p-3 text-xs leading-relaxed"} overflow-auto text-text-primary scrollbar-thin`}
      >
  ...
  return (
    <div className={embedded ? "space-y-1" : "space-y-1.5"}>
      {options.map((option) => (
        ...
        >
          <span
            className={`h-2 w-2 rounded-full ${value === option ? "bg-accent-primary" : "bg-border"}`}
          />
