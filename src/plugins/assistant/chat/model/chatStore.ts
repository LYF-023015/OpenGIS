...
} from "@/plugins/assistant/chat/data/chatPersistence";
import {
  createErrorMessagePart,
  createSystemTextMessagePart,
  createUserMessagePart,
  enqueueStreamingCodePart,
  flushBufferedCodePart,
  isStreamingCodePart,
  settleConversationForTurnEnd,
  settleRunningStatusCards,
  upsertNativePartIntoConversation,
} from "./chatMessageReducer";
...
async function configureBackendAgent(): Promise<void> {
  const { useSettingsStore } =
    await import("@/plugins/system/settings/model/settingsStore");
  const settings = useSettingsStore.getState();

  // 计算简单哈希值以检测更改
  const configStr = `${settings.model.protocol}|${settings.model.modelName}|${settings.model.apiKey}|${settings.model.baseURL || ""}|${settings.model.temperature}|${settings.model.maxTokens}`;
    ...
    switch (method) {
      case "chat.message_part": {
        ...
        if (part.type === "error") {
          ...
          if (conv)
            settleRunningStatusCards(conv, get()._updateMessage, "failed");
          if (activeConversationId) {
            set((s) => ({
              conversations: s.conversations.map((conversation) =>
                conversation.id === activeConversationId
        ...
        if (
          ...
          const metadata = (part.data?.metadata || {}) as Record<
            string,
            unknown
          >;
      ...
      // rpc.ui.map.* 通知由 src/shared/backend/rpc/ 处理函数消费，
      // 不是这里。
    }
  });
}
...
export const useChatStore = create<ChatStore>((set, get) => {
  ...
  return {
    ...
    deleteConversation: (id) => {
      // 从磁盘删除
      import("@/plugins/workspace/assets/model/assetStore").then(({ useAssetStore }) => {
      ...
      set((state) => {
        ...
        const hasWorkflowPlan =
          newActiveConv?.messages.some(
            (m) =>
              m.say === "plan" &&
              m.planData?.steps?.some((s: any) => s.status === "in_progress"),
          ) ?? false;
        return {
          conversations,
    ...
    resetConversation: () => {
      ...
      if (state.activeConversationId) {
        import("@/plugins/workspace/assets/model/assetStore").then(
          ({ useAssetStore }) => {
            const workspacePath = useAssetStore.getState().workspacePath;
            backendClient
              .send("rpc.agent.interrupt", {
    ...
    loadFromDisk: async () => {
      const { useAssetStore } =
        await import("@/plugins/workspace/assets/model/assetStore");
      const workspacePath = useAssetStore.getState().workspacePath;
      if (!workspacePath) {
    ...
    sendMessage: async (text, images, attachments) => {
      ...
      if (state.isStreaming || state.isCancelling) {
        console.warn(
          "[chatStore] sendMessage ignored while agent is busy/cancelling",
        );
        return;
      }
      //如果没有激活的对话，则创建一个新的对话
      ...
      const isFirstUserMessage =
        !conv || !conv.messages.some((m) => m.say === "user_feedback");
      ...
      try {
        ...
        if (!workspacePath) {
          const ts = Date.now();
          const text =
            "⚠️ 请先打开一个工作目录。Agent 需要工作目录来存储脚本和分析结果。";
          get()._addMessage({
            ts,
            type: "say",
            say: "error",
        ...
        const userInstructions =
          settingsStore.getState().agent.customInstructions || undefined;
        ...
        await backendClient.send("chat.user_message", {
          ...
          context_window: settingsStore.getState().model.contextWindow,
        });
      } catch (error: any) {
...
useAssetStore.subscribe((state, prev) => {
  if (state.workspacePath !== prev.workspacePath) {
    ...
    const settleCurrentRun =
      store.isStreaming || store.isCancelling
        ? store.abortTask(prev.workspacePath).catch((e) => {
            console.warn(
              "[chatStore] failed to abort active run before workspace switch:",
              e,
            );
          })
        : Promise.resolve();
