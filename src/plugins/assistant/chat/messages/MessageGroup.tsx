...
export function MessageGroup({
  ...
  if (role === "system") {
    return (
      <div
        className={`${highlighted ? "bg-accent-primary/5" : ""} px-5 transition-colors`}
      >
        {children}
      </div>
    );
  ...
  if (role === "user") {
    ...
    return (
      <div
        className={`${highlighted ? "bg-accent-primary/5" : ""} px-5 py-1.5 animate-fade-in group/msg transition-colors`}
      >
...
function extractGroupText(items: ChatMessage[]): string {
  const parts: string[] = [];
    ...
    if (messageParts.length > 0) {
      for (const part of messageParts) {
        if (part.type === "text" && part.text?.trim()) {
          ...
          part.text?.trim() &&
          !isCodeExecutionOutput(part) &&
          !isOperationToolOutput(part)
        ) {
          parts.push(part.text.trim());
        }
      }
...
function isOperationToolOutput(
  part: ReturnType<typeof messagePartsForRender>[number],
): boolean {
  return !!part.tool && OPERATION_TOOLS.has(part.tool);
}

function isCodeExecutionOutput(
  part: ReturnType<typeof messagePartsForRender>[number],
): boolean {
