...
import {
  ...
  type WorkflowNode,
  type WorkflowEdge,
} from "@/plugins/automation/workflows/model/workflow-schema";
...
export const useWorkflowStore = create<WorkflowState>((set, get) => ({
  ...
  renameWorkflow: async (oldPath: string, rawNewName: string) => {
    ...
    const dir = oldPath.substring(
      0,
      oldPath.length - basename(oldPath).length - 1,
    );
