...
export function parseWorkflow(
  ...
  if (version !== WORKFLOW_SCHEMA_VERSION) {
    throw new Error(
      `Workflow schema v${version} must be inspected and converted to v2 before editing.`,
    );
  }
  const now = new Date().toISOString();
...
function normaliseNode(node: any): WorkflowNode {
  ...
  return {
    ...
    execution: {
      ...
      ref: String(
        node?.execution?.ref ?? (type === "agent_task" ? "gis-build" : ""),
      ),
    },
    inputContract:
      typeof node?.inputContract === "string" ? node.inputContract : "",
    outputContract:
      typeof node?.outputContract === "string" ? node.outputContract : "",
    inputs: Array.isArray(node?.inputs) ? node.inputs.map(normalisePort) : [],
    outputs: Array.isArray(node?.outputs)
      ? node.outputs.map(normalisePort)
      : [],
    ...
    conditions: Array.isArray(node?.conditions)
      ? node.conditions.map(normaliseCondition)
      : [],
    retryPolicy: {
      maxAttempts: Math.max(
        1,
        Math.min(Number(node?.retryPolicy?.maxAttempts) || 1, 10),
      ),
      backoffMs: Math.max(
        0,
        Math.min(Number(node?.retryPolicy?.backoffMs) || 0, 60_000),
      ),
    },
...
function normalisePort(port: any): NodePort {
  return {
    ...
    label: stringOrUndefined(port?.label),
    type: stringOrUndefined(port?.type),
    description: stringOrUndefined(port?.description),
  };
}
...
function normaliseCondition(condition: any): WorkflowCondition {
  return {
    ...
    onFalse: ["fail", "retry", "skip"].includes(condition?.onFalse)
      ? condition.onFalse
      : "fail",
  };
}
...
function normaliseEdge(edge: any): WorkflowEdge {
  return {
    id: String(
      edge?.id ?? `edge_${String(edge?.source)}_${String(edge?.target)}`,
    ),
...
export function validateWorkflow(workflow: Workflow): void {
  ...
  for (const node of workflow.nodes) {
    if (!/^[A-Za-z0-9._-]+$/.test(node.id) || ids.has(node.id))
      throw new Error(`Invalid or duplicate node id: ${node.id}`);
    ids.add(node.id);
    if (node.type !== node.execution.kind || !node.execution.ref.trim())
      throw new Error(`Invalid execution reference for node ${node.id}`);
    if (node.execution.ref.toLowerCase().endsWith(".py"))
      throw new Error(
        `Python reference is not allowed in Workflow v2: ${node.id}`,
      );
    if (
      node.type === "java_script" &&
      !node.execution.ref.toLowerCase().endsWith(".java")
    )
      throw new Error(`java_script must reference a .java file: ${node.id}`);
  }
  for (const edge of workflow.edges) {
    if (!ids.has(edge.source) || !ids.has(edge.target))
      throw new Error(`Edge references a missing node: ${edge.id}`);
  }
}
