 ...
 * step 字段兼容历史 `steps.jsonl`，由当前 Java RunArchive 继续读写。
 */
...
function normalizeRunDetail(
  raw: RunDetailResponse | RunDetail | null | undefined,
): RunDetail | null {
  if (!raw) return null;
  const meta =
    "meta" in raw && raw.meta && typeof raw.meta === "object" ? raw.meta : raw;
...
export const useRunsStore = create<RunsStore>((set, get) => ({
  ...
  refreshControlPlane: async () => {
    ...
    try {
      if (workspacePath) {
        await backendClient.send("rpc.agent.queue.resume", {
  ...
  retryQueueItem: async (queueId) => {
    const workspacePath = useAssetStore.getState().workspacePath || undefined;
    await backendClient.send("rpc.agent.queue.retry", {
      queue_id: queueId,
      workspace_path: workspacePath,
    });
    await get().refreshControlPlane();
  ...
  cancelQueueItem: async (queueId) => {
    const workspacePath = useAssetStore.getState().workspacePath || undefined;
    await backendClient.send("rpc.agent.queue.cancel", {
      queue_id: queueId,
      workspace_path: workspacePath,
    });
    await get().refreshControlPlane();
  ...
  getDetail: async (runId, forceRefresh = false) => {
    ...
    try {
      const workspacePath = useAssetStore.getState().workspacePath || undefined;
      const raw = await backendClient.send<RunDetailResponse>("rpc.runs.get", {
        run_id: runId,
        workspace_path: workspacePath,
      });
