      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        ...
        {error ? (
          ...
          isLoading && !loaded ? (
            ...
            <EmptyState hasWorkspace={!!workspacePath} />
          )
        ) : (
          <div className="py-1">
            {runs.map((run) => (
...
function ControlPlaneSummary({
  ...
  const retryable = queueItems.filter(
  ...
  return (
    <div className="border-b border-border/70 px-2 py-2 space-y-1.5">
      ...
      {visibleQueue.length > 0 ? (
        <div className="space-y-1">
          {visibleQueue.map((item) => (
            <div key={item.id} className="flex items-center gap-1.5 text-2xs">
              ...
              <span
                className="font-mono text-text-muted/80 truncate flex-1"
        ...
        <div className="text-2xs text-text-muted/60">
          {t.runs.noQueuedItems}
        </div>
      )}
...
function RunRow({ run }: { run: RunSummary }) {
  ...
  const handleRevert = useCallback(
    async (e: React.MouseEvent) => {
      ...
      try {
        ...
        await alert({
          ...
          message: t.runs.workspaceRevertedMsg.replace(
            "{runId}",
    ...
    [
      ...
      t.runs.revertTitle,
      t.runs.workspaceReverted,
      t.runs.workspaceRevertedMsg,
    ],
  );
  ...
  const handleReplay = useCallback(
    ...
    [busy, replayRun, alert, run.run_id, t.runs.replayFailed],
  );
      ...
      >
        ...
        <div className="flex items-center gap-0.5 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <button
            ...
            title={
              run.pre_sha ? t.runs.revertTooltip : t.runs.noSnapshotTooltip
            }
          >
            {busy === "revert" ? (
              <RefreshCw className="w-3 h-3 animate-spin" />
        ...
        <div className="space-y-0.5">
          ...
          <div className="text-2xs text-text-muted/80 font-mono break-all">
            ...
            {detail.session.children && detail.session.children.length > 0 && (
              <span className="ml-2">
                children={detail.session.children.length}
              </span>
            )}
          </div>
        </div>
      ...
      {detail.tool_calls && detail.tool_calls.length > 0 && (
        <div className="space-y-0.5">
          ...
          {detail.tool_calls.slice(0, 8).map((call) => (
            <div
              key={call.call_id}
              className="text-2xs text-text-muted/80 font-mono break-all"
            >
              <span
                className={
                  call.error ? "text-accent-danger" : "text-accent-primary"
                }
              >
                {call.name}
              </span>
              ...
              {call.metadata?.permission != null && (
                <span className="ml-1.5 text-text-muted/60">
                  perm={String(call.metadata.permission)}
                </span>
              )}
            </div>
          ))}
...
function StatusDot({
  ...
  status: RunSummary["status"] | AgentWorkStatus;
}) {
  const color =
    ...
    <div className="flex-1 flex items-center justify-center p-4 h-full">
      <div className="text-center">
        ...
        <p className="text-2xs text-text-muted/60 max-w-[180px]">
          {hasWorkspace ? t.runs.noRunsHint : t.runs.noWorkspaceHint}
        </p>
      </div>
    </div>
  );
