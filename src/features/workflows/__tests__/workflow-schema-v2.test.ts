...
describe('Workflow schema v2', () => {
  it('creates and round-trips a structured Agent node', () => {
    ...
    workflow.nodes.push({
      id: 'step_1', title: 'Inspect data', type: 'agent_task',
      execution: { kind: 'agent_task', ref: 'gis-build' },
      inputs: [], outputs: [], params: {}, position: { x: 0, y: 0 },
      conditions: [{ expression: { exists: { var: 'output' } }, onFalse: 'fail' }],
      retryPolicy: { maxAttempts: 2, backoffMs: 100 },
    })
    const parsed = parseWorkflow(serialiseWorkflow(workflow))
    expect(parsed.schemaVersion).toBe(2)
    expect(parsed.nodes[0].execution).toEqual({ kind: 'agent_task', ref: 'gis-build' })
  })

  it('requires explicit migration for v1', () => {
    expect(() => parseWorkflow('{"schemaVersion":1,"nodes":[],"edges":[]}'))
  ...
  it('rejects Python and mismatched execution references', () => {
    ...
    workflow.nodes.push({
      id: 'step', title: 'Unsafe', type: 'agent_task',
    ...
    expect(() => validateWorkflow(workflow)).toThrow(/Invalid execution reference/)
  })
})
