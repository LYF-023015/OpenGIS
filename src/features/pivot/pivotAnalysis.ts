...
export function computePivotAnalysis(data: PivotData): PivotAgentResult {
  ...
  if (!table) return { stats: [], distributions: [], summary: 'No tabular sample is available.', engine: 'typescript' }
  ...
  const distributions = stats.filter((stat) => stat.count > 0).slice(0, 16)
    .map((stat) => computeDistribution(stat.field, stat.type, rows))
  const numeric = stats.filter((stat) => stat.type === 'number').length
  const categorical = stats.filter((stat) => stat.type === 'string' || stat.type === 'boolean').length
  return {
    stats,
    distributions,
...
export async function runPivotAgent(
  ...
  if (!backendClient.isConnected) {
    emit('Java backend is not connected; using deterministic renderer statistics.\n', 'error')
    return fallback
  }
  ...
  emit(`Sending structured Pivot request to Java (${payload.rows.length} rows).\n`)
  try {
    const reply = await backendClient.send<JavaPivotReply>('rpc.analysis.pivot', payload, 45_000)
    if (!Array.isArray(reply.stats) || !Array.isArray(reply.distributions) || typeof reply.summary !== 'string') {
    ...
    emit(`${message}\n`, 'error')
    return { ...fallback, summary: `${fallback.summary} Java analysis failed: ${message}` }
  }
}
...
function computeFieldStat(field: string, rows: Record<string, unknown>[]): PivotFieldStat {
  ...
  if (numericLikely) {
    return {
      field, type: 'number', count: nonNull.length, nullCount: values.length - nonNull.length,
      uniqueCount: unique.size, min: Math.min(...numeric), max: Math.max(...numeric),
      mean: numeric.reduce((sum, value) => sum + value, 0) / numeric.length,
    }
  }
  const booleanLikely = nonNull.length > 0 && nonNull.every((value) =>
    typeof value === 'boolean' || ['true', 'false'].includes(String(value).toLowerCase()))
  const strings = nonNull.map(String)
  return {
    field, type: booleanLikely ? 'boolean' : 'string', count: nonNull.length,
    nullCount: values.length - nonNull.length, uniqueCount: unique.size,
...
function computeDistribution(field: string, type: PivotFieldStat['type'], rows: Record<string, unknown>[]): PivotFieldDistribution {
  ...
  if (type === 'number') {
    ...
    if (min === max) return { field, type, buckets: [{ label: formatValue(min), count: numbers.length, probability: 1 }] }
    const size = Math.min(MAX_BUCKETS, Math.max(4, Math.round(Math.sqrt(numbers.length))))
    const counts = Array.from({ length: size }, () => 0)
    numbers.forEach((value) => counts[Math.min(size - 1, Math.floor(((value - min) / (max - min)) * size))]++)
    return { field, type, buckets: counts.map((count, index) => ({
      label: `${formatValue(min + ((max - min) * index) / size)}-${formatValue(min + ((max - min) * (index + 1)) / size)}`,
      count, probability: count / numbers.length,
    })) }
  }
  const counts = new Map<string, number>()
  values.forEach((value) => counts.set(String(value), (counts.get(String(value)) ?? 0) + 1))
  const buckets: PivotDistributionBucket[] = [...counts.entries()].sort((a, b) => b[1] - a[1]).slice(0, MAX_BUCKETS)
