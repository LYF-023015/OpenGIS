...
import { ArrowLeft, Map, Table } from "lucide-react";
import { useT } from "@/app/i18n";
import { Sidebar } from "./Sidebar";
import { ChatView } from "@/plugins/assistant/AssistantPlugin";
import {
  DataPivotPanel,
  MapView,
  OperationEditorView,
...
import {
  ...
  ImageViewer,
  ScriptRunnerView,
} from "@/plugins/workspace/WorkspacePlugin";
import { WorkflowEditorView } from "@/plugins/automation/AutomationPlugin";
import { useViewStore, type ViewTab } from "@/shell/model/viewStore";
...
export function MainLayout() {
  ...
  const sidebarContributions = useSidebarContributions();
  const activeContribution = sidebarContributions.find(
    (item) => item.id === activeSidebarTab,
  );
  ...
  const isMainView = activeContribution?.surface === "main";
  const isPrimaryView = activeContribution?.surface === "primary";
  ...
  const handleSidebarTabChange = useCallback(
    (tab: string) => {
      const contribution = sidebarContributions.find((item) => item.id === tab);
      contribution?.onActivate?.();
      if (contribution?.surface !== "panel") {
    ...
    [activeSidebarTab, sidebarContributions],
  );
  ...
  const showSidebarContent =
    sidebarContentVisible && activeContribution?.surface === "panel";
  const openMapWorkspace = () => {
    setActiveSidebarTab("layers");
    setSidebarContentVisible(false);
  };
  const renderContext: UiRenderContext = { openMapWorkspace };
    ...
    <div className="relative h-screen w-screen overflow-hidden select-none">
      ...
      >
        ...
        <Sidebar
          ...
          isContentVisible={
            showSidebarContent || activeContribution?.surface !== "panel"
          }
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          {isMainView && activeContribution ? (
            <div className="flex-1 overflow-hidden">
              {activeContribution.render(renderContext)}
            </div>
          ) : (
            <>
              ...
              <PanelGroup direction="horizontal" className="flex-1">
                ...
                <Panel defaultSize={showChat ? 68 : 100} minSize={30}>
                  <PrimaryPanel
                    ...
                    primaryContent={
                      isPrimaryView && activeContribution
                        ? activeContribution.render(renderContext)
                        : undefined
                    }
                  />
...
function BoardModeShell({
  ...
  return (
    <div
      className={`board-mode-shell absolute inset-0 z-[9000] overflow-hidden bg-bg-primary p-3 sm:p-4 ${exiting ? "board-mode-shell--exit" : ""}`}
    >
...
function ResizableSidebarPanel({ activeTab }: { activeTab: string }) {
  ...
  return (
    <div
      className="h-full border-r border-border shrink-0 relative flex"
      style={{ width }}
    >
      <div className="flex-1 overflow-hidden">
...
function SidebarContent({ activeTab }: { activeTab: string }) {
  const contributions = useSidebarContributions();
  const contribution = contributions.find((item) => item.id === activeTab);
  return contribution?.surface === "panel"
    ? contribution.render({ openMapWorkspace: () => {} })
    : null;
}
...
function StatusBar() {
  ...
  const featureCount =
    activeLayer?.data?.kind === "vector" ? activeLayer.data.featureCount : 0;
  ...
  return (
    <div className="h-6 bg-bg-secondary border-t border-border flex items-center px-3 text-2xs text-text-muted select-none">
      ...
      {activeLayer && (
        <>
          <span className="mr-4">
            {featureCount.toLocaleString()} {t.layout.features}
          </span>
...
/** 底部属性表当前只提供空状态，和工作台外壳放在一起避免形成单文件功能目录。 */
function DataTable() {
  const t = useT();
...
function PrimaryPanel({
  ...
  const viewTabs = tabs.filter(
    (t) => t.type === "code" || t.type === "text" || t.type === "image",
  );
...
function CodeTabContent({ tab }: { tab: ViewTab }) {
  ...
  const isImage =
    lang === "image" || /\.(png|jpe?g|gif|bmp|webp|svg)$/i.test(path);
  if (isImage) {
