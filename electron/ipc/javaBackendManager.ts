...
export class JavaBackendManager implements BackendProcessManager {
  ...
  getStatus(): BackendStatus {
    return { ...this.status, diagnostics: [...(this.status.diagnostics ?? [])] }
  }
  ...
  async start(): Promise<void> {
    if (this.status.status === 'starting' || this.status.status === 'ready') return
    ...
    this.logStream = openLogFile(`java-sidecar-${new Date().toISOString().slice(0, 10)}.log`)
    const args = [
      '-jar', paths.serverPath,
      '--host', '127.0.0.1',
      '--port', String(this.port),
      '--log-dir', logDir,
    ]
    this.logStream?.write(`${timestamp()} [INFO] starting ${paths.javaPath} ${args.join(' ')}\n`)
    ...
    this.child.stdout?.on('data', (value: Buffer) => this.handleOutput(value, 'OUT'))
    this.child.stderr?.on('data', (value: Buffer) => this.handleOutput(value, 'ERR'))
  ...
  async stop(): Promise<void> {
    ...
    await new Promise<void>((resolve) => {
      ...
      if (process.platform === 'win32' && child.pid) {
        execFile('taskkill', ['/T', '/PID', String(child.pid)], { timeout: 5_000 }, () => undefined)
      } else {
        child.kill('SIGTERM')
      }
      setTimeout(() => {
        if (child.exitCode === null && child.signalCode === null) {
          if (process.platform === 'win32' && child.pid) {
            execFile('taskkill', ['/F', '/T', '/PID', String(child.pid)], { timeout: 5_000 }, () => done())
          } else {
            child.kill('SIGKILL')
            done()
          }
  ...
  private resolveRuntimePaths(): { javaPath: string; serverPath: string } {
    if (app.isPackaged) {
      return {
        javaPath: join(process.resourcesPath, 'java-runtime', 'bin', executable('java')),
        serverPath: join(process.resourcesPath, 'java-backend', 'opengis-server.jar'),
      }
    }
    const root = join(__dirname, '../..')
    const bundledJava = join(root, 'java-backend', 'opengis-server', 'target', 'runtime', 'bin', executable('java'))
    const serverPath = join(root, 'java-backend', 'opengis-server', 'target', 'opengis-server-0.1.0-SNAPSHOT.jar')
    return {
  ...
  private validateRuntime(javaPath: string, serverPath: string): string[] {
    ...
    if (app.isPackaged && !existsSync(javaPath)) diagnostics.push(`Bundled Java runtime is missing: ${javaPath}`)
    if (!existsSync(serverPath)) diagnostics.push(`OpenGIS Java server is missing: ${serverPath}`)
    try {
      if (existsSync(serverPath) && readFileSync(serverPath, { encoding: null }).length === 0) {
  ...
  private handleOutput(value: Buffer, stream: 'OUT' | 'ERR'): void {
    ...
    this.logStream?.write(`${timestamp()} [${stream}] ${output}${output.endsWith('\n') ? '' : '\n'}`)
    ...
    if (trimmed) (stream === 'ERR' ? console.error : console.log)(`[Java] ${trimmed}`)
  }

  private handleProcessError(error: Error): void {
    this.status = { ...this.status, status: 'error', error: `Failed to start Java backend: ${error.message}` }
    this.broadcastStatus()
  }

  private handleExit(code: number | null, signal: NodeJS.Signals | null): void {
    this.logStream?.write(`${timestamp()} [INFO] process exited code=${code} signal=${signal}\n`)
    this.child = null
    this.closeLog()
    ...
    setTimeout(() => {
      if (!this.stopping) this.start().catch((restartError) => console.error('[Java] restart failed', restartError))
  ...
  private waitForReady(timeoutMs: number): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      const check = () => {
        ...
        if (this.status.status === 'error') return reject(new Error(this.status.error ?? 'Java backend failed'))
        if (Date.now() - startedAt >= timeoutMs) return reject(new Error(`Java backend startup timed out after ${timeoutMs}ms`))
  ...
  private findAvailablePort(): Promise<number> {
    return new Promise((resolve, reject) => {
      ...
      server.listen(0, '127.0.0.1', () => {
        ...
        if (!address || typeof address === 'string') return reject(new Error('Unable to allocate backend port'))
        server.close(() => resolve(address.port))
      })
    })
  }

  private async recordLastGood(javaPath: string, serverPath: string): Promise<void> {
    const target = join(app.getPath('userData'), LAST_GOOD_FILE)
    const temporary = `${target}.tmp`
    const checksum = createHash('sha256').update(readFileSync(serverPath)).digest('hex')
