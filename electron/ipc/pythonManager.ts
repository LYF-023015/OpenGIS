    ...
    try {
      ...
      this.process.stderr?.on("data", (data: Buffer) => {
        ...
        if (
          ...
          // Notify renderer
          this.sendToRenderer("backend:status-changed", {
            ...this.getStatus(),
            runtime: "python",
          });
      ...
      this.process.on("exit", (code) => {
        ...
        if (this.restartCount < this.maxRestarts && !this.isRestarting) {
          ...
          console.log(
            `[Python] Auto-restarting (${this.restartCount}/${this.maxRestarts}) in ${this.restartDelayMs}ms...`,
          );
          this.status = { status: "starting" };
          this.sendToRenderer("backend:status-changed", {
            ...this.getStatus(),
      ...
      this.process.on("error", (err) => {
        ...
        this.stdoutMirror?.write(
          `${ts()} [ERROR] spawn error: ${err.message}\n`,
        );
        this.status = {
          status: "error",
    ...
    if (existsSync(venvPython)) {
      try {
        ...
        if (result.includes("Python 3")) {
          console.log(
            `[Python] Using project venv: ${venvPython} (${result.trim()})`,
          );
          return venvPython;
        }
      } catch {
    ...
    return new Promise((resolve, reject) => {
      ...
      const check = () => {
        ...
        if (this.status.status === "error") {
          reject(
            new Error(this.status.error || "Python backend failed to start"),
          );
          return;
        }
