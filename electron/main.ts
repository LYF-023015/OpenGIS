...
function registerBackendIpcHandlers(): void {
  ...
  ipcMain.handle("backend:status", () => {
    return (
      backendManager?.getStatus() ?? { status: "stopped", runtime: "java" }
    );
  });
  ...
  while (true) {
    try {
      ...
      updateLoadingProgress(
        3,
        `${backendManager.runtime === "java" ? "Java" : "Development Python"} backend connected!`,
      );
      ...
      const parentWindow =
        loadingWindow?.isDestroyed() === false ? loadingWindow : null;
...
app
  ...
  .then(async () => {
    ...
    ipcMain.on("renderer:ready", () => {
      ...
      if (
        (global as any).__projectSelected &&
        (global as any).__selectedProject
      ) {
        mainWindow?.webContents.send(
          "project:selected",
          (global as any).__selectedProject,
    ...
    ipcMain.handle("system:open-log-dir", async () => {
      const dir = getLogDir();
      if (!dir)
        return { success: false, error: "Log directory not initialised" };
    ...
    ipcMain.handle(
      "file:write-binary",
      (_event, filePath: string, buffer: ArrayBuffer) => {
        try {
    ...
    if (
      (global as any).__projectSelected &&
      (global as any).__rendererIsReady &&
      loadingWindow &&
        ...
        if (backendManager) {
          ...
          if (status.status === "stopped" || status.status === "error") {
            try {
              ...
              console.error(
                "[main] Failed to restart backend on activate:",
                err,
              );
            }
          }
