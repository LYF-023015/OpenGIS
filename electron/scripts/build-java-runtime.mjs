...
const server = join(javaBackend, "server");
...
if (!resolve(runtime).startsWith(`${resolve(target)}${sep}`))
  throw new Error(`Unsafe runtime output: ${runtime}`);

run(wrapper, [
  "-q",
  "-f",
  join(javaBackend, "pom.xml"),
  "-pl",
  "opengis-server",
  "-am",
  "install",
...
run(wrapper, [
  ...
  "-DoutputDirectory=target/jlink-libs",
]);
run(wrapper, [
  "-q",
  "-f",
...
const javaHome =
  ...
  properties.match(/^\s*java\.home\s*=\s*(.+)$/m)?.[1]?.trim();
if (!javaHome)
  throw new Error("Unable to resolve a JDK from JAVA_HOME or java.home");

const tool = (name) =>
  join(javaHome, "bin", process.platform === "win32" ? `${name}.exe` : name);
const jars = readdirSync(libraries)
  .filter((name) => name.endsWith(".jar"))
  .map((name) => join(libraries, name));
