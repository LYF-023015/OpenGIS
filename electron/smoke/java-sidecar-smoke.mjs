...
function waitForExit(child, timeoutMs) {
  return Promise.race([
    ...
    new Promise((_, reject) =>
      setTimeout(
        () => reject(new Error("Java Sidecar stop timeout")),
        timeoutMs,
      ),
    ),
  ]);
...
try {
  ...
  while (!stdout.includes("OPENGIS_READY") && Date.now() < deadline) {
    if (child.exitCode !== null)
      throw new Error(`Java Sidecar exited ${child.exitCode}: ${stderr}`);
  ...
  if (!response.ok || health.status !== "ok" || health.version !== "0.1.0") {
    throw new Error(
      `Invalid health response: ${response.status} ${JSON.stringify(health)}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Invalid RPC response: ${rpcResponse.status} ${JSON.stringify(rpc)}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Invalid Phase 3 RPC response: ${phase3Response.status} ${JSON.stringify(phase3Rpc)}`,
    );
  }
  const fixtureWorkspace = join(
    repoRoot,
    "test",
    "phase0",
    "fixtures",
  ...
  const phase4CatalogResponse = await fetch(
  ...
  if (
    ...
    throw new Error(
      `Invalid Phase 4 catalog: ${phase4CatalogResponse.status} ${JSON.stringify(phase4CatalogRpc)}`,
    );
  }
  const phase4ExecuteResponse = await fetch(
    `http://127.0.0.1:${port}/api/rpc`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  if (
    ...
    throw new Error(
      `Invalid Phase 4 execution: ${phase4ExecuteResponse.status} ${JSON.stringify(phase4ExecuteRpc)}`,
    );
  }
  const phase5ProvidersResponse = await fetch(
    `http://127.0.0.1:${port}/api/rpc`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  if (
    ...
    !phase5ProvidersRpc.result.providers.some(
    ...
    throw new Error(
      `Invalid Phase 5 Provider catalog: ${phase5ProvidersResponse.status} ${JSON.stringify(phase5ProvidersRpc)}`,
    );
  }
  ...
  if (!phase5CacheResponse.ok || phase5CacheRpc.result?.requests !== 0) {
    throw new Error(
      `Invalid Phase 5 cache diagnostics: ${phase5CacheResponse.status} ${JSON.stringify(phase5CacheRpc)}`,
    );
  }
  const phase6InspectResponse = await fetch(
    `http://127.0.0.1:${port}/api/rpc`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  const phase6IssueCodes =
    phase6InspectRpc.result?.issues?.map((issue) => issue.code) ?? [];
  if (
    ...
    throw new Error(
      `Invalid Phase 6 migration inspection: ${phase6InspectResponse.status} ${JSON.stringify(phase6InspectRpc)}`,
    );
  }
  ...
  if (
    !phase6SaveResponse.ok ||
    phase6SaveRpc.result?.workflow_id !== phase6Workflow.id
  ) {
    throw new Error(
      `Invalid Phase 6 workflow save: ${phase6SaveResponse.status} ${JSON.stringify(phase6SaveRpc)}`,
    );
  }
  ...
  if (
    !phase6LoadResponse.ok ||
    phase6LoadRpc.result?.workflow?.schemaVersion !== 2
  ) {
    throw new Error(
      `Invalid Phase 6 workflow load: ${phase6LoadResponse.status} ${JSON.stringify(phase6LoadRpc)}`,
    );
  }
  ...
  if (!phase6QueueResponse.ok || phase6QueueRpc.result?.status !== "queued") {
    throw new Error(
      `Invalid Phase 6 queue submit: ${phase6QueueResponse.status} ${JSON.stringify(phase6QueueRpc)}`,
    );
  }
  const phase6CancelResponse = await fetch(`http://127.0.0.1:${port}/api/rpc`, {
    ...
    body: JSON.stringify({
      ...
      params: {
        workspace_path: logDir,
        queue_id: phase6QueueRpc.result.queue_id,
      },
    }),
  });
  const phase6CancelRpc = await phase6CancelResponse.json();
  if (
    !phase6CancelResponse.ok ||
    phase6CancelRpc.result?.status !== "cancelled"
  ) {
    throw new Error(
      `Invalid Phase 6 queue cancellation: ${phase6CancelResponse.status} ${JSON.stringify(phase6CancelRpc)}`,
    );
  }
  ...
  const phase7CapabilitiesResponse = await fetch(
    `http://127.0.0.1:${port}/api/rpc`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  if (
    ...
    throw new Error(
      `Invalid Phase 7 capabilities: ${phase7CapabilitiesResponse.status} ${JSON.stringify(phase7CapabilitiesRpc)}`,
    );
  }
  const phase7InspectResponse = await fetch(
    `http://127.0.0.1:${port}/api/rpc`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  if (
    ...
    throw new Error(
      `Invalid Phase 7 GIS inspection: ${phase7InspectResponse.status} ${JSON.stringify(phase7InspectRpc)}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Invalid Phase 9 Pivot RPC: ${phase9PivotResponse.status} ${JSON.stringify(phase9PivotRpc)}`,
    );
  }
  const phase8OperationsResponse = await fetch(
    `http://127.0.0.1:${port}/api/rpc`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  const phase8OperationIds =
    phase8OperationsRpc.result?.operations?.map((operation) => operation.id) ??
    [];
  if (
    ...
    throw new Error(
      `Invalid Phase 8 Operation catalog: ${phase8OperationsResponse.status} ${JSON.stringify(phase8OperationsRpc)}`,
    );
  }
...
public final class SmokeScript implements OpenGisScript {
  ...
  if (
    ...
    throw new Error(
      `Invalid Phase 8 Script execution: ${phase8ScriptResponse.status} ${JSON.stringify(phase8ScriptRpc)}`,
    );
  }
...
public final class SmokeWorker implements OpenGisWorker {
  ...
  if (!phase8WorkerResponse.ok || typeof phase8WorkerId !== "string") {
    throw new Error(
      `Invalid Phase 8 Worker start: ${phase8WorkerResponse.status} ${JSON.stringify(phase8WorkerRpc)}`,
    );
  }
  ...
  if (
    !phase8PauseResponse.ok ||
    phase8PauseRpc.result?.worker?.status !== "paused"
  ) {
    throw new Error(
      `Invalid Phase 8 Worker pause: ${phase8PauseResponse.status} ${JSON.stringify(phase8PauseRpc)}`,
    );
  }
  console.log(
    JSON.stringify({
