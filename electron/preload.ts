...
const electronAPI = {
  ...
  readDirectory: (path: string) => ipcRenderer.invoke("file:read-dir", path),
   ...
   */
  ensureDirectory: (path: string) => ipcRenderer.invoke("file:mkdir", path),
  ...
  getBackendWsToken: () => ipcRenderer.invoke("backend:get-ws-token"),
  ...
  browseProjectFolder: () => ipcRenderer.invoke("projects:browse-folder"),

  switchProject: () => ipcRenderer.invoke("app:switch-project"),
  ...
  openLogDir: () =>
    ipcRenderer.invoke("system:open-log-dir") as Promise<{
      success: boolean;
      path?: string;
      error?: string;
    }>,
